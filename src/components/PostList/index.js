...
export const PostList = ({ like, setLike, userInfo, getPost }) => {
  ...
  const newAdded = () => {
    setPosts(
      posts.sort(function (a, b) {
        return dayjs(b.created_at).format("YYYYMMDDHHmmss") - dayjs(a.created_at).format("YYYYMMDDHHmmss");
      })
    );
    navigate("/");
  };
  const oldAdded = () => {
    setPosts(
      posts.sort(function (a, b) {
        return dayjs(a.created_at).format("YYYYMMDDHHmmss") - dayjs(b.created_at).format("YYYYMMDDHHmmss") ;
      })
    );
    navigate("/");
  };
